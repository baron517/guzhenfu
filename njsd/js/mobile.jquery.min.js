...
(function(a) {
    function b(b, c, d) {
        ...
        a.isFunction(c[g]) && f.bind(g, c[g]), a.isFunction(c[h]) && f.bind(h, c[h]), a.extend(e, {
            ...
            _init: function() {
                ...
                if (e.length < 1) { window.console && console.warn("No panel in " + d); return }
                ...
                if (!!c.triggers) {
                    ...
                    if (c.slide) {
                        var n = e.triggers.eq(e.index),
                            w = (n.width() + parseInt(n.css("padding-left")) + parseInt(n.css("padding-right"))) / 2 + n.position().left - 5;
                        n.siblings("span").animate({ left: w }, 100)
                    }
                    for (g = 0; g < e.length; g++) {
                        f = e.triggers.eq(g), f.click({ index: g }, function(a) {
                            h = a.data.index;
                            ...
                            e._delayTimer = setTimeout(function() {
                                if (c.slide) {
                                    var w = (n.width() + parseInt(n.css("padding-left")) + parseInt(n.css("padding-right"))) / 2 + n.position().left - 5;
                                    n.siblings("span").animate({ left: w }, 100)
                                }
                                e.switchTo(h)
                            }, c.delay * 1000)
                        }).mouseleave(function() { e._cancelDelayTimer() })
                    }
                }
            },
            _triggerIsValid: function(a) { return e.index !== a },
            _cancelDelayTimer: function() { e._delayTimer && (clearTimeout(e._delayTimer), e._delayTimer = undefined) },
            ...
            _switchPanels: function(b, d, f) { a.switchable.Effects[c.effect].call(e, b, d, f) },
            willTo: function(a) { return a ? e.index > 0 ? e.index - 1 : c.loop ? e.length - 1 : !1 : e.index < e.length - 1 ? e.index + 1 : c.loop ? 0 : !1 },
    ...
    a.switchable = {
        Config: { triggers: !0, putTriggers: "insertAfter", triggersWrapCls: "switchable-triggers", currentTriggerCls: "current", panels: null, steps: 1, triggerType: "mouse", delay: 0.1, initIndex: 0, effect: "none", easing: "ease", duration: 0.5, loop: !0, beforeSwitch: null, onSwitch: null, api: !1, slide: false, roll: false, numm: 10, lazyload: false },
        Effects: {
            none: function(a, b) {
                var c = this,
                    d = c.config;
                c.panels.slice(a * d.steps, (a + 1) * d.steps).hide().end().slice(b * d.steps, (b + 1) * d.steps).show();
                if (d.slide || d.lazyload) {
                    c.panels.slice(b * d.steps, (b + 1) * d.steps).find("img.x_lazy").each(function() {
                        var original = $(this).attr("data-original");
                        $(this).attr("src", original).addClass("imgcompleted")
                    })
                }
            }
        },
...
function(a) {
    function b() {
        var a = document.documentElement,
            ...
            d = "",
            e;
        if (a.style[c] !== undefined) { d = c } else { for (e = 0; e < 2; e++) { if (a.style[c = b[e] + "Transition"] !== undefined) { d = c; break } } }
        return d
    }
    a.switchable.Anim = function(c, d, e, f, g, h) {
        ...
        a.switchable.Transition === undefined && (a.switchable.Transition = b()), k = a.switchable.Transition, a.extend(i, {
            ...
            run: function() {
                ...
                e = e * 1000;
                if (k) { j[k + "Property"] = h || "all", j[k + "Duration"] = e + "ms", j[k + "TimingFunction"] = f, c.css(a.extend(d, j)), l = setTimeout(function() { i._clearCss(), i._complete() }, e) } else {
                    var b = /cubic-bezier\(([\s\d.,]+)\)/,
                        g = f.match(b),
                        m = a.switchable.TimingFn[f];
                    if (m || g) { f = a.switchable.Easing(g ? g[1] : m.match(b)[1]) }
                    c.animate(d, e, f, function() { i._complete() })
                }
                return i.isAnimated = !0, i
            },
            stop: function(a) { if (!i.isAnimated) { return } return k ? (clearTimeout(l), l = undefined) : c.stop(!1, a), i.isAnimated = !1, i },
            _complete: function() { g && g() },
            _clearCss: function() { j[k + "Property"] = "none", c.css(j) }
        })
    }
}(jQuery),
function(a) {
    function b(a) { return "cubic-bezier(" + a + ")" }
    ...
    function d(a, b, c, d, e, f) {
        function h(a) { return ((g * a + bx) * a + cx) * a }

        function i(a) { return ((ay * a + by) * a + cy) * a }

        function j(a) { return (3 * g * a + 2 * bx) * a + cx }

        function k(a) { return 1 / (200 * a) }

        function l(a, b) { return i(m(a, b)) }

        function m(a, b) {
            function k(a) { return a >= 0 ? a : 0 - a }
            var c, d, e, f, g, i;
            for (e = a, i = 0; i < 8; i++) {
                f = h(e) - a;
                if (k(f) < b) { return e }
                g = j(e);
                if (k(g) < 0.000001) { break }
                e = e - f / g
            }
            c = 0, d = 1, e = a;
            if (e < c) { return c }
    ...
    a.switchable.TimingFn = { ease: b(".25, .1, .25, 1"), linear: b("0, 0, 1, 1"), "ease-in": b(".42, 0, 1, 1"), "ease-out": b("0, 0, .58, 1"), "ease-in-out": b(".42, 0, .58, 1") }, a.switchable.Easing = function(e) {
        ...
        if (g !== 4) { window.console && console.warn(b(e.join(", ")) + " missing argument.") } else {
            ...
            if (!a.easing[f]) {
                var i = c(function(a) { return d(a, e[0], e[1], e[2], e[3], 5) });
                a.easing[f] = function(a, b, c, d) { return i.call(null, a) }
            }
        }
        return f
    }
...
function(a) {
    a.extend(a.switchable.Config, { prev: null, next: null }), a.switchable.Plugins.push({
        ...
        init: function(b) {
            ...
            for (; i < 2; i++) {
                f = e[i], g = c[f], g && (h = b[f + "Btn"] = g.jquery ? g : a(g), h.click({ direction: d[i] }, function(a) {
                    a.preventDefault();
                    if (!b.anim) {
                        ...
                        if (custom.roll) {
                            ...
                            function Change() {
                                ...
                                if (b.index <= num2) { animate(oListUl, { left: 0 }) } else { if (b.index + num2 <= len2) { animate(oListUl, { left: -(b.index - num2) * w2 }) } else { animate(oListUl, { left: -(len2 - num) * w2 }) } }
                            }
                            Change()
                        }
                    }
...
function(a) {
    a.switchable.Effects.fade = function(b, c) {
        var d = this,
            ...
            g = f.eq(b),
            h = f.eq(c);
        d.anim && (d.anim.stop(), f.eq(d.anim.to).css({ zIndex: d.length }).end().eq(d.anim.from).css({ opacity: 0, zIndex: 1 })), h.css({ opacity: 1 }), d.anim = (new a.switchable.Anim(g, { opacity: 0 }, e.duration, e.easing, function() { h.css({ zIndex: d.length }), g.css({ zIndex: 1 }), d.anim = undefined }, "opacity")).run(), d.anim.from = b, d.anim.to = c
    ...
    for (var f = 0; f < 4; f++) {
        a.switchable.Effects[b[f]] = function(b, c, d) {
            var e = this,
                ...
                j = {};
            j[e.isHoriz ? "left" : "top"] = i ? e._adjustPosition(h) : -e.groupSize[e.isHoriz ? 0 : 1] * c, e.anim && e.anim.stop(), e.anim = (new a.switchable.Anim(e.panels.parent(), j, f.duration, f.easing, function() { i && e._resetPosition(h), e.anim = undefined })).run()
        }
    }
    a.switchable.Plugins.push({
        name: "scroll effect",
        init: function(f) {
            ...
            if (g.end2end) {
                var s = i.length,
                    ...
                    x;
                g.loop = !0, g.visible && g.visible < s && g.visible > u && i.slice(0, g.visible).clone(!0).addClass(g.clonedCls).appendTo(j).click(function(b) { b.preventDefault(), i.eq(a(this).index() - s).click() }), a.extend(f, { _adjustPosition: function(a) { return x = a ? p : 0, r[c] = e, r[q] = (a ? -1 : 1) * t, i.slice(x * h, (x + 1) * h).css(r), a ? w : -t }, _resetPosition: function(a) { x = a ? p : 0, r[c] = "", r[q] = "", i.slice(x * h, (x + 1) * h).css(r), r[c] = undefined, r[q] = a ? -f.groupSize[o] * p : 0, j.css(r) } })
            }
    ...
    a.switchable.Plugins.push({
        ...
        init: function(d) {
            ...
            window.console && console.info("Remember to set the border-width for the accordion's panels, even without border."), a.extend(d, {
                _triggerIsValid: function(a) { return !0 },
                _switchTrigger: function(a, b) {
                    var c = d.triggers,
...
(function($, window, document, undefined) {
    ...
    $.fn.lazyload = function(options) {
        var elements = this;
        var $container;
        var settings = { threshold: 0, failure_limit: 0, event: "scroll", effect: "show", container: window, data_attribute: "original", skip_invisible: true, appear: null, load: null, placeholder: "//img.xhdxly.com/public/index.gif", placeholderClass: "x_lazy" };
            ...
            $self.one("appear", function() {
                if (!this.loaded) {
                    ...
                    $("<img />").bind("load", function() {
                        ...
                        if ($self.hasClass("imgcompleted")) { return } else {}
                        $self.hide();
...
var login = function() {
    return location.href = "https://user.xialingying.cc/accounts/login?u=" + escape(location.href).replace(/\//g, "%2F"), !1
};
var regist = function() {
    return location.href = "https://user.xialingying.cc/accounts/register?u=" + escape(location.href), !1
};
var consultation = function() {

    //window.open("http://vipwebchat.tq.cn/sendmain.jsp?admiuin=9348474&action=acd&tag=%3Cype=1&rand=48669955359245889&iscallback=0&agentid=0&type_code=1");
    doyoo.util.openChat('g=10077925');
    return false;
    ...
    $.extend({
        publicapp: {
            ...
            mobilegoBack: function() {
                if ($('#mobilegoback').length <= 0) {
                    return;
                }
                var u = navigator.userAgent;
                var goBack = document.getElementById("mobilegoback");
                var yuanurl = 'index.html';
                var locationhref = window.location.href;
                //针对ios原生浏览器处理
                //if(!!u.match(/\(i[^;]+;( U;)? CPU.+Mac OS X/) && /(Safari)/i.test(u)){
                if (/MSIE (\d+\.\d+);/.test(navigator.userAgent) || /MSIE(\d+\.\d+);/.test(navigator.userAgent)) {
                    if (locationhref == document.referrer || document.referrer === '') {
                        goBack.setAttribute("onclick", "javascript:location.href='" + yuanurl + "';");
                    } else {
                        if (history.length > 0) {
...
(function(factory) { if (typeof define === 'function' && define.amd) { define(['jquery'], factory) } else { factory(jQuery) } }(function($, undefined) {
        ...
        if (this.o.indicators && this.$indicators.length && !this.$indicators.children().length) {
            ...
            for (i = 0; i < this.o.steps; i++) {
                if (this.options.word) {
                    this.$indicators.append('<li><i></i><span>' + this.$items.eq(i + 1).find('img').attr("alt") + '</span></li>')
                } else {
    ...
    Plugin.prototype.move = function(num, public, init) {
        ...
        this.$inner.animate(position, {
            ...
            complete: function() {
                //+ 2015.6.1
                init && _this.$element.css('visibility', 'visible')
